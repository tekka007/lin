...
static struct resource bigphysarea_resource = { 
	.name	= "bigphysarea",
	.start	= 0,
	.end	= 0,
...
int __init bigphysarea_setup(char *str)
{
	...
	if (get_option(&str,&par)) {
		...
		bigphysarea = alloc_bootmem(bigphysarea_pages<<PAGE_SHIFT);
		if (!bigphysarea) {
		...
		bigphysarea_resource.start = virt_to_phys(bigphysarea);
		bigphysarea_resource.end = 
			bigphysarea_resource.start + (bigphysarea_pages<<PAGE_SHIFT)-1;
		insert_resource(&iomem_resource, &bigphysarea_resource);
	}
	return 1;
}
...
int bigphysarea_reserved_pages(void)
{
	return bigphysarea_pages;
}
EXPORT_SYMBOL(bigphysarea_reserved_pages);
