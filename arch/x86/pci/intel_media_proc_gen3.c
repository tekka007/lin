...
static int gen3_conf_read3(unsigned int seg, unsigned int bus,
			  unsigned int devfn, int reg, int len, u32 *value)
{
	unsigned long flags;
	unsigned int dev;
	...
	if (sim_reg != NULL) {
		...
		*value = sim_reg->value;
		raw_spin_unlock_irqrestore(&pci_config_lock, flags);
                ...
                /* EHCI registers has 0x100 offset. */
		if (bus == 1 && dev == 13 && reg == 0x10 && func < 4) {
			if (*value != sim_reg->mask) {
				*value |= 0x100;
			}
		}
...
static int gen3_conf_write3(unsigned int seg, unsigned int bus,
			   unsigned int devfn, int reg, int len, u32 value)
{
	unsigned long flags;
	int dev;
...
static int gen3_conf_read5(unsigned int seg, unsigned int bus,
			  unsigned int devfn, int reg, int len, u32 *value)
{
	unsigned long flags;
	unsigned int dev;
	...
	if (sim_reg != NULL) {
		...
		*value = sim_reg->value;
		raw_spin_unlock_irqrestore(&pci_config_lock, flags);
                ...
                /* EHCI registers has 0x100 offset. */
		if (bus == 1 && dev == 13 && reg == 0x10 && func < 3) {
			if (*value != sim_reg->mask) {
				*value |= 0x100;
			}
		}
	...
	DBG("gen3_conf_read5: %2x:%2x.%d[%2X(%d)] = %X\n", bus, dev, func, reg, len, *value);
	return retval;
}

static int gen3_conf_write5(unsigned int seg, unsigned int bus,
			   unsigned int devfn, int reg, int len, u32 value)
{
	unsigned long flags;
	int dev;
	...
	DBG("gen3_conf_write5: %2x:%2x.%d[%2X(%d)] <- %X\n", bus, dev, func, reg, len, value);
...
struct pci_raw_ops gen3_pci_conf3 = {
	.read =	gen3_conf_read3,
	.write = gen3_conf_write3,
};
struct pci_raw_ops gen3_pci_conf5 = {
	.read =	gen3_conf_read5,
	.write = gen3_conf_write5,
};


static int __init gen3_pci_init(void)
{
	unsigned int root_devid = 0, pcimode = 0;
	...
	}  else if ( root_devid == CE4200_DEVID ){
